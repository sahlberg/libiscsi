...
{ 
	...
	printf("==============================\n");
	if (show_info) {
		printf("Test VERIFY12 BYTCHK:0 only checks medium and should never fail unless the medium is bad.\n");
	...
	task = iscsi_read12_sync(iscsi, lun, 0, 256 * block_size, block_size, 0, 0, 0, 0, 0);
	if (task == NULL) {
	        printf("[FAILED]\n");
...
test2:
	...
	for (i = 1; i <= 256; i++) {
		task = iscsi_verify12_sync(iscsi, lun, NULL, i * block_size, 0, 0, 1, 0, block_size);
		if (task == NULL) {
		        printf("[FAILED]\n");
