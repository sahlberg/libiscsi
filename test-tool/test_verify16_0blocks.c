...
test_verify16_0blocks(void)
{
	...
	ret = verify16(sd->iscsi_ctx, sd->iscsi_lun, 0, 0, block_size,
		       0, 0, 1, NULL,
		       EXPECT_STATUS_GOOD);
	if (ret == -2) {
	...
	ret = verify16(sd->iscsi_ctx, sd->iscsi_lun, num_blocks + 1, 0,
	...
	ret = verify16(sd->iscsi_ctx, sd->iscsi_lun, 0x8000000000000000ULL, 0,
		       block_size, 0, 0, 1, NULL,
		       EXPECT_LBA_OOB);
	CU_ASSERT_EQUAL(ret, 0);
	...
	ret = verify16(sd->iscsi_ctx, sd->iscsi_lun, -1, 0, block_size,
		       0, 0, 1, NULL,
		       EXPECT_LBA_OOB);
	CU_ASSERT_EQUAL(ret, 0);
}
