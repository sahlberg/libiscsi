...
test_writeverify12_0blocks(void)
{
	...
	ret = writeverify12(sd->iscsi_ctx, sd->iscsi_lun, 0, 0, block_size,
			    0, 0, 0, 0, NULL,
			    EXPECT_STATUS_GOOD);
	if (ret == -2) {
	...
	ret = writeverify12(sd->iscsi_ctx, sd->iscsi_lun, num_blocks + 1, 0,
	...
	ret = writeverify12(sd->iscsi_ctx, sd->iscsi_lun, 0x80000000, 0,
			    block_size, 0, 0, 0, 0, NULL,
			    EXPECT_LBA_OOB);
	CU_ASSERT_EQUAL(ret, 0);
	...
	ret = writeverify12(sd->iscsi_ctx, sd->iscsi_lun, -1, 0, block_size,
			    0, 0, 0, 0, NULL,
			    EXPECT_LBA_OOB);
	CU_ASSERT_EQUAL(ret, 0);
}
