...
{ 
	...
	for (i = 1; i <= 256; i++) {
		...
		ret = write12(sd->iscsi_ctx, sd->iscsi_lun, num_blocks + 1 - i,
	...
	for (i = 1; i <= 256; i++) {
		...
		ret = write12(sd->iscsi_ctx, sd->iscsi_lun, 0x80000000,
			      i * block_size, block_size, 0, 0, 0, 0, 0, buf,
			      EXPECT_LBA_OOB);
		CU_ASSERT_EQUAL(ret, 0);
	}
	...
	for (i = 1; i <= 256; i++) {
		...
		ret = write12(sd->iscsi_ctx, sd->iscsi_lun, -1, i * block_size,
	...
	for (i = 2; i <= 256; i++) {
		...
		ret = write12(sd->iscsi_ctx, sd->iscsi_lun, num_blocks - 1,
			      i * block_size, block_size, 0, 0, 0, 0, 0, buf,
			      EXPECT_LBA_OOB);
		CU_ASSERT_EQUAL(ret, 0);
	}
